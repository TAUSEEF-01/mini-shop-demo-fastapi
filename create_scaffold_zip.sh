...
# ROOT_DIR="mini-shop-demo-fastapi"
# ZIP_NAME="${ROOT_DIR}.zip"

# echo "Creating scaffold in ./\$ROOT_DIR and packaging as \$ZIP_NAME..."
...
#   cat > "$ROOT_DIR/$filepath" <<'EOF'
# '"$content"'
# EOF
# }

# # Because heredoc above would literally write "$content", we'll instead use a function to write direct here-documents per file.
# # Create files directly with here-docs:
...
# - Implement JWT auth & password hashing, ObjectId handling, robust Kafka header-based trace propagation, retries and DLQ, tests, CI workflow, and production hardening.
...
# mkdir -p "$ROOT_DIR/user-service/app" "$ROOT_DIR/product-service/app" "$ROOT_DIR/order-service/app" "$ROOT_DIR/payment-service/app" "$ROOT_DIR/notification-service/app"

# # user-service files
# cat > "$ROOT_DIR/user-service/Dockerfile" <<'EOF'
...
# cat > "$ROOT_DIR/user-service/requirements.txt" <<'EOF'
# fastapi
# uvicorn[standard]
# motor
# pydantic
...
# cat > "$ROOT_DIR/user-service/app/main.py" <<'EOF'
...
# cat > "$ROOT_DIR/user-service/app/db.py" <<'EOF'
...
# cat > "$ROOT_DIR/user-service/app/handlers.py" <<'EOF'
...
# cat > "$ROOT_DIR/product-service/Dockerfile" <<'EOF'
# FROM python:3.11-slim
...
# cat > "$ROOT_DIR/product-service/requirements.txt" <<'EOF'
# fastapi
# uvicorn[standard]
# motor
# pydantic
...
# cat > "$ROOT_DIR/product-service/app/main.py" <<'EOF'
...
# cat > "$ROOT_DIR/product-service/app/db.py" <<'EOF'
# from motor.motor_asyncio import AsyncIOMotorClient
# from typing import Optional
...
# cat > "$ROOT_DIR/product-service/app/cache.py" <<'EOF'
# import aioredis
# from typing import Optional
...
# cat > "$ROOT_DIR/product-service/app/handlers.py" <<'EOF'
...
# # order-service files
# cat > "$ROOT_DIR/order-service/Dockerfile" <<'EOF'
# FROM python:3.11-slim
...
# cat > "$ROOT_DIR/order-service/requirements.txt" <<'EOF'
# fastapi
# uvicorn[standard]
# motor
# pydantic
...
# cat > "$ROOT_DIR/order-service/app/main.py" <<'EOF'
# from fastapi import FastAPI
# import os
# from .db import init_db, get_db
# from .handlers import register_routes
...
# cat > "$ROOT_DIR/order-service/app/db.py" <<'EOF'
# from motor.motor_asyncio import AsyncIOMotorClient
# from typing import Optional
...
# cat > "$ROOT_DIR/order-service/app/handlers.py" <<'EOF'
...
# cat > "$ROOT_DIR/payment-service/Dockerfile" <<'EOF'
# FROM python:3.11-slim
...
# cat > "$ROOT_DIR/payment-service/requirements.txt" <<'EOF'
# fastapi
# uvicorn[standard]
# motor
# pydantic
...
# cat > "$ROOT_DIR/payment-service/app/main.py" <<'EOF'
# from fastapi import FastAPI
# import os
...
# cat > "$ROOT_DIR/payment-service/app/handlers.py" <<'EOF'
# import asyncio
# import os
...
# cat > "$ROOT_DIR/payment-service/app/db.py" <<'EOF'
# from motor.motor_asyncio import AsyncIOMotorClient
# from typing import Optional
...
# cat > "$ROOT_DIR/notification-service/Dockerfile" <<'EOF'
# FROM python:3.11-slim
...
# cat > "$ROOT_DIR/notification-service/requirements.txt" <<'EOF'
# fastapi
# uvicorn[standard]
# aiokafka
# opentelemetry-api
...
# cat > "$ROOT_DIR/notification-service/app/main.py" <<'EOF'
# from fastapi import FastAPI
# import os
# from .handlers import start_consumer
...
# cat > "$ROOT_DIR/notification-service/app/handlers.py" <<'EOF'
# import asyncio
# import os
...
# # Minimal Loki & promtail config placeholders (if user wants to edit later)
# cat > "$ROOT_DIR/loki-config.yml" <<'EOF'
# auth_enabled: false
# server:
...
# cat > "$ROOT_DIR/promtail-config.yml" <<'EOF'
# server:
...
# echo
# echo "Generated files and top-level structure:"
# tree -a -I "__pycache__" "$ROOT_DIR" || true

# echo
# echo "Done. You can now upload $ZIP_NAME to GitHub or unzip it."
# EOF